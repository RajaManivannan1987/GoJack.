...
/**
 * Created by Im033 on 3/30/2017.
 */

public class HailTrip extends CommonNavigstionBar implements GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener,
        LocationListener, PlaceSelectionListener, OnMapReadyCallback {
    ...
    //Copy of hailActivity variables
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        dialog = new ProgressDialog(HailTrip.this);
    ...
    private void onClick() {
        hailDirectionButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (toAddressLatLng != null) {
                    ...
                    CommonMethods.locationDirection(HailTrip.this, lat, lang);
                } else {
                    CommonMethods.toast(HailTrip.this, "Enter your destination");
                }
            }
        });
        SwipeButtonCustomItems startSwipeButton = new SwipeButtonCustomItems() {
            ...
            public void onSwipeConfirm() {
                final ProgressDialog progressBar = new ProgressDialog(HailTrip.this);
                        ...
                        public void onResponse(JSONObject response) throws JSONException {
                            if (response.getString("token_status").equalsIgnoreCase("1")) {
                                if (response.getString("status").equalsIgnoreCase("1")) {
                                    ...
                                    CommonMethods.toast(HailTrip.this, response.getString("message"));
                                    rideid = response.getString("rideid");
                                    ridetype = response.getString("ridetype");
                                    PreferenceManager.getDefaultSharedPreferences(HailTrip.this).edit().putString(CommonIntent.rideId, rideid).commit();
                                    PreferenceManager.getDefaultSharedPreferences(HailTrip.this).edit().putString(CommonIntent.rideType, ridetype).commit();
                                    rideStatus = response.getString("status");
                                    tripStatus = "2";
                                    //  CommonMethods.toast(HailActivity.this, response.getString("message"));
                                    // flag = true;
                                    if (mCurrentLocation != null) {
                                        updateUI(prefManager.getGender());
                                    }
        ...
        SwipeButtonCustomItems endSwip = new SwipeButtonCustomItems() {
            @Override
            public void onSwipeConfirm() {
                ProgressDialog progressBar = new ProgressDialog(HailTrip.this);
                progressBar.setMessage("Waiting for response...");
                progressBar.setCancelable(false);
                progressBar.show();
                String address = CommonMethods.getMarkerMovedAddress(HailTrip.this, new LatLng(gpsTracker.getLatitude(), gpsTracker.getLongitude()));
                if (!address.equalsIgnoreCase("")) {
                    progressBar.dismiss();
                    final String RideId = PreferenceManager.getDefaultSharedPreferences(HailTrip.this).getString(CommonIntent.rideId, "0");
                    final String RideType = PreferenceManager.getDefaultSharedPreferences(HailTrip.this).getString(CommonIntent.rideType, "ride");

                    WebServiceClasses.getWebServiceClasses(HailTrip.this, TAG).hideEndTrip(RideId, currentLat, currentLong, address, new VolleyResponseListerner() {
                        @Override
                        public void onResponse(JSONObject response) throws JSONException {
                            tripStatus = "0";
                            CommonMethods.toast(HailTrip.this, "Trip Completed");
                        ...
                        @Override
                        public void onError(String message, String title) {
                            AlertDialogManager.showAlertDialog(HailTrip.this, title, message, false);
                        }
                    });
                }
            }
    ...
    private void findViewById() {
        ...
        prefManager = PrefManager.getPrefManager(HailTrip.this);
        ...
        autocompleteFragment.setOnPlaceSelectedListener(HailTrip.this);
            ...
            case GoJackServerUrls.MY_PERMISSIONS_REQUEST_LOCATION:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED)
                        if (!CommonMethods.checkProvider(HailTrip.this)) {
                            CommonMethods.showLocationAlert(HailTrip.this);
                        }
                } else {
                    CommonMethods.checkLocationPermission(HailTrip.this);
                }
        }
    }
    ...
    public void onPlaceSelected(Place place) {
        toAddress = CommonMethods.getMarkerMovedAddress(HailTrip.this, place.getLatLng());
...
//        String str = getMarkerMovedAddress(place.getLatLng(), "toaddress");
    ...
    private void updateUI(String genderType) {
        if (marker != null) {
            marker.remove();
        }
    ...
    private void animateMarker(final Marker marker, final LatLng currentLocation, final boolean hideMarker) {
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                float t = interpolator.getInterpolation((float) elapsed
                        / duration);
                double lng = t * currentLocation.longitude + (1 - t)
                        * startLatLng.longitude;
                double lat = t * currentLocation.latitude + (1 - t)
...
//        updateUI(pr);
    ...
    private void checkRideStatus() {
        WebServiceClasses.getWebServiceClasses(HailTrip.this, TAG).checkRideStatus(new VolleyResponseListerner() {
            @Override
            public void onResponse(JSONObject response) throws JSONException {
    ...
    private void checkBuildPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (CommonMethods.checkLocationPermission(HailTrip.this)) {
                if (!CommonMethods.checkProvider(HailTrip.this)) {
                    CommonMethods.showLocationAlert(HailTrip.this);
                }
            }
            ...
            public void onResult(LocationSettingsResult locationSettingsResult) {
                ...
                switch (status.getStatusCode()) {
                    case LocationSettingsStatusCodes.SUCCESS:
                        ...
                        if (ActivityCompat.checkSelfPermission(HailTrip.this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(HailTrip.this, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                            return;
                        }
                        LocationServices.FusedLocationApi.requestLocationUpdates(
                                mGoogleApiClient, mLocationRequest, HailTrip.this);
                        break;
                    case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                        ...
                        try {
                            ...
                            status.startResolutionForResult(HailTrip.this, REQUEST_CHECK_SETTINGS);
                    ...
                    case LocationSettingsStatusCodes.SETTINGS_CHANGE_UNAVAILABLE:
                        ...
                        Toast.makeText(HailTrip.this, errorMessage, Toast.LENGTH_LONG).show();
                        mRequestingLocationUpdates = false;
                }
                updateUI(prefManager.getGender());
            }
